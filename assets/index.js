...
const AWAL_PERSENTASE_MENANG = 95;
...
function AlgoritmaJudi() {
	const uangPemain = conf.player.money;

	let persentaseMenang =  clamp(AWAL_PERSENTASE_MENANG - (uangPemain / 4), 20, 90);

	return persentaseMenang * Math.floor((Math.random() * 100) + 1) <= persentaseMenang;
}
...
const Slot = function(canvas){
	...
	this.setJackpot = function() {
		JackpotDalam = conf.jackpotElement.value;
	...
	this.spin = function(){
		...
		const ApakahMenang = JackpotDalam === 0 || AlgoritmaJudi();
		console.log(ApakahMenang, AlgoritmaJudi());
		
		reels.forEach(reel => {
			reel.clicked = true;
			reel.finalAnimation = false;
			reel.finalShapes = ApakahMenang ? reel.generateReelShapesFixed() : reel.setFinalShapes();
			currentSpin.push(reel.finalShapes);
		});

		JackpotDalam--;
		if(JackpotDalam < 0) {
			JackpotDalam = Math.floor((Math.random() * 30) + 1)
		}
		conf.jackpotElement.value = JackpotDalam;
	...
	this.loop = function( now ) {
		reels.forEach((reel, i)=> {
			...
			if(delta > conf.reel.animTimes[i] && reel.spinning && conf.sudahKlik){
	...
	this.drawStopPoints = function(){
		...
		[60,120,180].forEach(x => {
			this.canvas.strokeRect(0, x, 5, 0);
...
let JackpotDalam = conf.jackpotElement.value;
let BerapaKaliSpinSebelumJackpot = 0;
...
Resources(
	...
	.onLoad(function(resources, names){
		...
		(function(slot){
			...
			conf.jackpotElement.onchange = slot.setJackpot;
